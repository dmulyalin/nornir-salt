...
def _run_test(
    ...
    :patam test_function: callable function to use for testing
    :param test_type: (str) type of test running
    :param use_all_task: (bool) default is False, passes all results to
    ...
    for hostname, results in result.items():
        ...
        # get task results by task name
            ...
            if serialize:
                task_result = results.get(task, task_result)
            else:
                for i in results:
                    if i.name == task:
                        task_result = i
...
def ContainsTest(
    ...
    :param count: (int) exact number of pattern occurrences in the output
    ...
    def _contains(task_result, pattern, use_re, criteria, count, **kwargs):
        ...
        elif count and task_result["result"].count(pattern) != count:
            ret["result"] = "FAIL"
            ret["success"] = False
            ret["error"] = "Criteria pattern not in output {} times".format(count)
    ...
    def _not_contains(task_result, pattern, use_re, criteria, count, **kwargs):
        ret = {"criteria": criteria}
        if use_re:
            ...
            ret["result"] = "FAIL"
            ret["success"] = False
            ret["error"] = "Criteria pattern in output {} times".format(count)
...
def ContainsLinesTest(
    ...
    :param result: ``nornir.core.task.AggregatedResult`` object
    :param pattern: (str or list) lines to check, if string, lines can be
        separated using ``\\n`` newline, e.g.: "line1\\nline2\\nline3"
    :param task: (str or int) name of task to check results for,
        can be list index instead, default is ``-1``, last task's results
    :param test_name: (str) descriptive name of the test
    ...
    def _not_contains_lines(task_result, pattern, **kwargs):
        ret = {"criteria": ""}
        lines_list = pattern.splitlines() if isinstance(pattern, str) else pattern
        for line in lines_list:
            if not line.strip():
...
def CustomFunctionTest(
    ...
    :param result: ``nornir.core.task.AggregatedResult`` object if ``serialize``
        is False, else dictionary produced by ``ResultSerializer``. If
        ``use_all_task`` set to True, ``result`` contains results for all tasks
        run for this host, otherwise ``result`` contains results only for ``task``.
    :param serialize: (bool) default False, if True will serialize
        results to dictionary using ``ResultSerializer`` function
    ...
    :param use_all_task: (bool) default is False, if True passes all hosts's results
        to custom function.
    ...
    Notes on ``use_all_task`` argument. If ``use_all_task`` set to False (default),
    only results (``nornir.core.task.Result`` object) for ``task`` specified
    passed to test function, if ``use_all_task`` is True all host's results
...
def RunTestSuite(
    ...
    :param brief: (bool) default False, if True will return dictionary keyed by test name
        with nested dictionary keyed by host name with values set to True of False
    ...
    * ``CustomFunctionTest`` calls ``CustomFunctionTest``
    ...
    if tabulate and HAS_TABULATE:
        ...
        new_ret = {}
        for i in ret:
            new_ret.setdefault(i["test_name"], {})
            new_ret[i["test_name"]][i["host"]] = i["success"]
        ret = new_ret
