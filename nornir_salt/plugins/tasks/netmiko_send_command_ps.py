...
def send_command_ps(
        ...
        if not hasattr(self, "prompts_seen"):
            ...
            time.sleep(3) # need to wait for device to emit find_prompt() output so that self.clear_buffer() can catch it
            log.debug("send_command_ps formed prompts_seen set: {}".format(self.prompts_seen))
        ...
        while True:
            ...
            matched = get_close_matches(new_last_line.strip(), list(self.prompts_seen), 1, cutoff)
            log.debug("send_command_ps: last line '{}' similar prompt - '{}'".format(new_last_line.strip(), matched))
            if matched:
