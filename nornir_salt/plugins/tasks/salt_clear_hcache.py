...
def salt_clear_hcache(task: Task, cache_keys: list = None, **kwargs) -> Result:
    ...
    log.debug("nornir-salt:salt_clear_hcache removing hcache keys '{}'".format(cache_keys))
    ...
    for key in cache_keys:
        if key in task.host.data and key in task.host.data.get("_hcache_keys_", []):
            ...
            log.debug("nornir-salt:salt_clear_hcache removed hcache key '{}'".format(key))
            result[key] = True
        else:
            result[key] = False
