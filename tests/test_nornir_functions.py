...
ntp server 7.7.7.7
    ...
    check_result = ContainsTest(
        ...
        task="check ntp config",
        pattern="ntp server 7.7.7.8",
        tabulate={"headers": ["host", "test_name", "result"]},
    )
    # print(check_result)
    assert (
        check_result
        == """host    test_name         result
------  ----------------  --------
...
def test_cerberus_dict():
    output = nr.run(
        task=nr_test,
        ret_data_per_host={"IOL1": {"mtu": 1200}, "IOL2": {"mtu": 1500}},
        name="interfaces MTU",
    )
    test_schema = {"mtu": {"type": "integer", "allowed": [1500]}}
...
Loopback0                  10.0.0.7        YES NVRAM  up                    up
    ...
    assert res == {
        "router-1": {
            "show ip int brief": "Ethernet0/0.2000           192.168.217.10  YES NVRAM  up                    up",
            "show run": " ip address 192.168.217.10 255.255.255.0",
        },
        "router-2": {
            "show ip int brief": "Ethernet0/0.2000           192.168.217.7   YES NVRAM  up                    up",
